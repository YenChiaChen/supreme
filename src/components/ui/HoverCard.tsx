...
const HoverCard: React.FC<HoverCardProps> = ({  data }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0  hidden group-hover:flex opacity-0 px-12 mt-24  text-white ${
      ...
      >
        ...
        <FontAwesomeIcon icon={data.icon} className="h-[50px] mb-6" />
          ...
          <hr />
          <p className='text-[15px] font-light tracking-wide mt-6'>{data.description}</p>
        </div>
      </div>
    </div>
  );
