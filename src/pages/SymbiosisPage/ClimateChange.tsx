...
import BgImage from "../../assets/img/subPage/永續共生/氣候變遷對策.jpg";
import Breadcrumbs from "../../components/nav/BreadCrumbs";
import navItems from "../../data/nav.json";
import { H2, P } from "../../components/ui/Text";
import { Image } from "../../components/ui";
import FlowChartImage from '../../assets/img/subPage/永續共生/climate-change-flowchart.png'
import MatrixImage from '../../assets/img/subPage/永續共生/climate-change-matrix.png'
...
const ClimateChange: React.FC = () => {
  const AnimatedListItems1 = [
    {
      id: 1,
      content: "由永續發展 - 風險管理組負責招集各部門代表（含：財務、會計、業務、管理、倉儲、資訊、人資、法務）共同進行氣候相關議題討論及蒐整，經確認各項議題之風險及對組織內部和外部的衝擊，包含氣候變遷風險之鑑別評估及因應氣候衝擊。以擇定公司當年度永續發展優先倡議，包含氣候變遷相關議題，以實際行動履行永續政策。 在內部控制制度之監督下，由各單位依據法令規範及風險管理之執行，並透過內部稽核實施查核，以確保營運安全及穩定，並定期每年一次向董事會報告績效成果，使董事會能及時掌握永續相關趨勢以及公司在氣候治理議題上的執行進度與因應狀況，以期更能落實所制定之氣候變遷政策。",
      title: "・治理",
      titleColor:"#3BC376"
    },
    {
      id: 2,
      content: "各單位依循 TCFD 指引進行完整的氣候變遷相關風險與機會之鑑別，透過分析產業概況以及國際永續趨勢，將氣候議題區分為實體風險、轉型風險與氣候機會三大類別，並針對各項風險機會的影響期程、發生可能性與衝擊程度進行評估，最終確認四項轉型風險、二項實體風險及四項氣候機會，以擬定相關目標與因應策略，未來每年將定期監督實施情形與目標達成度，以達管理目的。",
      title: "・策略",
      titleColor:"#3BC376"
    },
  ];
    ...
    <div className="mb-48">
      ...
      <Container className="mt-12">
        ...
        <Section>
          ...
          <P
            text={
              "本公司參照「氣候相關財務揭露」( Task Force on Climate-related Financial Disclosures, TCFD ) 架構，透過「治理」、 「策略」、「風險管理」、「指標與目標」等四項核心要素，辨識潛在氣候變遷風險與機會，掌握對營運的衝擊 與影響，訂定相關因應策略與措施，降低氣候變遷所帶來的損失。"
            }
          />
          <div className="mt-8">
        ...
        <Section>
          <H2 text={"氣候相關風險與機會之鑑別流程"} />
          <Image src={FlowChartImage} alt="Flow Chart" />
        </Section>
        <Section>
          <H2 text={"氣候風險與機會重大性矩陣圖"} />
          <Image src={MatrixImage} alt="Flow Chart" />
        </Section>
        <Section>
          <H2 text={"氣候相關風險及管理程序"} />
          <table className="min-w-full border-collapse">
              <thead className="bg-green text-white content">
                <tr>
                  <th className="py-2 px-4 border border-white">類別</th>
              ...
              <tbody>
                <tr className="content bg-white">
                  <td className="py-2 px-4 border border-light_gray whitespace-pre-wrap text-nowrap" rowSpan={4}>轉型風險</td>
                  <td className="py-2 px-4 border border-light_gray whitespace-pre-wrap ">R2對新技術的投資失敗</td>
                  <td className="py-2 px-4 border border-light_gray whitespace-pre-wrap text-nowrap">中長期</td>
